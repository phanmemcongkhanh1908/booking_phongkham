    ...
    api.get('/admin/settings').then(res => {
      ...
      if (dbBookingFormConfig) {
        ...
        setBookingFormConfig(prev => ({
          ...
          preVisitNotes: Array.isArray(configParsed.preVisitNotes) ? configParsed.preVisitNotes : DEFAULT_PRE_VISIT_NOTES,
        }));
      }
      if (dbEmailConfig) {
          ...
          <form onSubmit={handleSaveBookingForm} className="space-y-6">
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50/50 p-4 sm:p-5 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-200/80 pb-3">
                ...
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  onClick={handleResetDefaultNotes}
                  className="text-xs flex items-center gap-1.5 self-start sm:self-auto cursor-pointer"
                >
                  <RotateCcw className="w-3.5 h-3.5 text-slate-500" />
                  Mặc định ban đầu
            ...
            <div className="rounded-2xl border border-dashed border-teal-200 bg-teal-50/30 p-4 sm:p-5 space-y-3">
              ...
              <div className="rounded-xl bg-white p-4 border border-slate-200/90 shadow-xs space-y-4 text-xs">
                ...
                {/* Pre-visit Notes Preview */}
                <div className="space-y-1.5 pt-1 border-t border-slate-100">
                  <div className="font-bold text-slate-700 flex items-center gap-1.5">
                    <ShieldCheck className="w-3.5 h-3.5 text-teal-600" />
                    Lưu ý trước khi đến khám ({bookingFormConfig.preVisitNotes?.length || 0}):
                  </div>
                  <div className="space-y-1 pt-1">
                    {(bookingFormConfig.preVisitNotes || []).map((note: string, idx: number) => {
                      const parts = note.split(/\*\*(.*?)\*\*/g);
                      return (
                        <div key={idx} className="flex items-start gap-1.5 text-[11px] text-slate-600">
                          <span className="text-teal-600 font-bold">•</span>
                          <span>
                            {parts.map((p, i) => (i % 2 === 1 ? <strong key={i} className="text-slate-800 font-semibold">{p}</strong> : p))}
                          </span>
                        </div>
                      );
                    })}
